...
| 2026-04-19 | TD     | Chunk 2.5q shipped (fourth of seven wizard-redesign chunks). Loud autosave + keyboard shortcuts + revert-last-change behind `WIZARD_V2_ENABLED`. New service method `QuestionDraftService.autosave(actor, draftId, step, patch)` merges the step's parsed patch into the draft's payload without advancing `current_step` and without emitting a `question.draft.advanced` audit row (autosave fires every few seconds and would drown the audit log); the normal `advance` path keeps emitting as before. New route `POST /admin/questions/wizard/:draftId/step/:n/autosave` re-uses the existing `parseWizardStep` dispatcher, returns 204 on success, 422 with `{ ok: false, issues: StepIssue[] }` on validation failure, 403 when the actor isn't the owner, 400 without CSRF. Client `src/static/v2/wizard_autosave.js` binds to `[data-wizard-editor][data-autosave-url]`, debounces `input` by 800 ms, flushes immediately on `change` for radios/checkboxes/selects and on `blur` (capture-phase so every field bubbles up), updates the sticky action-bar chip's `data-state` through `saved`→`saving`→`error`/`offline`, retries with exponential backoff on failure (cap 30 s), flushes once on the `online` event, and exposes `window.__wizardAutosavePulse()` so `wizard_shortcuts.js` can trigger a manual save without re-reading the form. Revert-last-change folded into the autosave script (no schema change needed): captures `priorSnapshot` of the form body just before each successful POST, injects a `data-wizard-revert` button into the chip area, hides it after 10 s via `REVERT_WINDOW_MS`, and on click re-POSTs the captured prior state plus restores the in-DOM field values via `CSS.escape` name lookup. Client `src/static/v2/wizard_shortcuts.js` listens on `document.keydown`: `Ctrl/Cmd+Enter` submits the step form (works inside typing contexts), `Ctrl/Cmd+S` calls `__wizardAutosavePulse` (preventDefault), `?` opens the help dialog, `/` focuses the nearest search/combobox/text input, `.` focuses the first `[aria-invalid="true"]` field (falls back to focusing the error summary), `[`/`]` navigates to the previous/next step via the rail's link (so the shortcut respects the "already-visited" gate — disabled steps have no link), `Esc` outside a typing context and outside an open dialog navigates back to `/admin/questions/wizard`. The handler ignores keystrokes inside `INPUT`/`TEXTAREA`/`SELECT`/`contenteditable` unless Alt is held or the key is a form-submit accelerator. New template `src/templates/v2/_wizard_shortcut_help_dialog.eta` renders a native `<dialog>` populated from an inline `SHORTCUTS` array — the dialog and the dispatcher reference the same canonical key labels so future keybinding edits only need to touch two files. CSS additions (~90 lines in `src/static/site.css`): `.wizard__shortcut-help` + `::backdrop` + `-list` + `-row`, `.wizard__shortcut-plus`, `.wizard__revert-btn` (dashed-border ghost, brand-50 on hover). Scripts wired into `_chrome.eta` via the existing `wizardV2ShellEnabled` flag. Per-step partials `_wizard_step_<n>.eta` remain untouched. `src/routes/admin-question-wizard.ts` gains the autosave route after the advance POST; no changes to existing routes. New tests: `tests/http/wizard-v2/autosave.test.ts` (7 cases): 204 on valid step-4 patch with payload merge but no `current_step` bump, no `question.draft.advanced` audit row emitted, missing CSRF → 400/403, cross-teacher access → 403, invalid step-6 patch → 422 with `issues[]` body, script served + `data-autosave-url` wired into the form, autosave after a mid-wizard resume preserves `current_step` across multiple calls. `tests/http/wizard-v2/shortcuts.test.ts` (4 cases): dialog renders on every v2 step with every documented `<kbd>`, `wizard_shortcuts.js` is served and the dispatch table mentions each case, site.css ships the `.wizard__shortcut-help` + `.wizard__revert-btn` primitives, v1 (flag off) does not include the dialog or the shortcut script. Final: 90 files / 820 tests passing (full `npm run check` green post-prettier auto-format). Unblocks chunk 2.5r (steps 1–3 upgrades: cascading combobox + command-word chip grid + widget tiles with 60×60 SVG thumbnails).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-04-19 | TD     | Chunk 2.5r shipped (fifth of seven wizard-redesign chunks). Steps 1–3 upgrades behind `WIZARD_V2_ENABLED`. New v2 partials `src/templates/v2/_wizard_step_1.eta` (cascading combobox: same three native selects, plus a `data-wizard-curriculum-filter` search input that narrows every `<option>` and hides empty optgroups), `_wizard_step_2.eta` (command-word as a `.wizard__chip` radio grid — `data-wizard-chip-grid` with native `<input type="radio" name="command_word_code">` wrapped in `<label>`, archetype remains a `<select>`), `_wizard_step_3.eta` (widget tiles now include a 60×60 `widget-tile__thumb` `<img>`, auto-marked / teacher-review coloured badge, and a "Your recent widgets" promotion section above the recommended grid when the author has a prior draft). Shell body `_admin_wizard_step_body.eta` branches per-step includes: steps 1–3 resolve to `./_wizard_step_<n>` (v2 folder), 4–9 continue resolving to `../_wizard_step_<n>` (v1 untouched). New widget-registry field `WidgetDescriptor.thumbUrl` plus helper `widgetThumbUrl(type)` = `/static/widget_thumbs/<type>.svg` — deterministic per type, so every registered response type has a thumb without per-entry boilerplate. 17 new placeholder SVGs (60×60 viewBox, `currentColor` stroke/fill so dark mode themes automatically) under `src/static/widget_thumbs/` one per `EXPECTED_RESPONSE_TYPES` entry; `docs/widgets.schema.json` regenerated. New repo method `QuestionDraftRepo.recentWidgetsByAuthor(authorUserId, limit)` aggregates the author's drafts by `payload->>'expected_response_type'`, orders by `MAX(updated_at) DESC`, returns the top N type codes; exposed on the service as `recentWidgetsForActor(actor, limit)`. Route `renderStep` accepts an optional `recentWidgets` array and passes it through to the template; the GET step handler fetches it via the service when `n===3 && isWizardV2Enabled()`. New static scripts `src/static/v2/wizard_curriculum_filter.js` (debounce-free live filter on option text + optgroup hide when all kids hidden; no-ops when the filter input or selects aren't present) and `src/static/v2/wizard_command_word_grid.js` (`:has()` fallback `is-selected` toggling on radio change, pulses `window.__wizardAutosavePulse()` so chip clicks save immediately rather than waiting 800 ms for the debounce). Both wired into `_chrome.eta` under the existing `wizardV2ShellEnabled` flag. CSS additions (~90 lines in `src/static/site.css`): `.wizard__curriculum-filter`, `.wizard__chip-field`, `.wizard__chip-grid` (CSS-grid with `auto-fill` 160 px min), `.wizard__chip` (hover + focus-visible + `:has(input:checked)` highlight in brand colour), `.wizard__chip-label` / `-sub`, `.widget-tile-section` + `__heading`, `.widget-tile__thumb` (60×60, `currentColor` so tile state colours the glyph), `.widget-tile__marking--deterministic` vs `--teacher_pending` (success-600 vs warn-700). New tests: `tests/unit/widget-thumbs.test.ts` (2 cases — every `EXPECTED_RESPONSE_TYPES` entry has a file on disk containing `<svg` + `currentColor`; `widgetThumbUrl()` matches the descriptor field for every registered widget), `tests/http/wizard-v2/step-1-3.test.ts` (9 cases — step 1 renders the filter + three cascading selects behind the flag / omits filter when flag off, step 2 renders the chip grid with `name="command_word_code"` radios + archetype `<select>` / omits the chip grid when flag off, step 3 tiles include the thumb `<img>` + both marking-badge variants, "Your recent widgets" section appears when the author has a prior `matching`-widget draft / is absent on a fresh author, both v2 scripts served via `/static/v2/`, every thumb SVG returns 200 with an `svg` content-type). Final: 92 files / 831 tests passing (full `npm run check` green post-prettier auto-format). Unblocks chunk 2.5s (steps 4–9 upgrades: writing canvas + slash menu + mark-point editor + difficulty slider + paper-preview polish + publish confirm).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 2026-04-19 | TD     | Chunk 2.5s shipped (sixth of seven wizard-redesign chunks). Steps 4, 8, 9 upgrades behind `WIZARD_V2_ENABLED` + shared enhancements script. New v2 partials `src/templates/v2/_wizard_step_4.eta` (live `data-wizard-char-counter` displaying `<stem length> / 4000 characters` with `aria-live="polite"` on the counter element; server renders the initial value so the counter is correct even before JS loads) and `_wizard_step_8.eta` (difficulty-band native range slider `data-wizard-difficulty-range` coupled with a `visually-hidden` `<select>` that remains the form-submit source and the no-JS fallback; source radios promoted to the same `.wizard__chip` chip-grid primitive step 2 introduced, with "Teacher-authored / Imported / AI-generated" chips carrying sub-captions). Shell-body publish button on step 9 now wears `data-wizard-hold-confirm data-hold-duration="1200" data-hold-label-{idle,progress,done}` attributes; the form itself gains `data-wizard-publish-form` for test targeting. New static script `src/static/v2/wizard_step_enhancements.js` wires all three behaviours: (a) char counters mirror `field.value.length` into `#<display-id> [data-char-count]` on every `input` event; (b) difficulty slider mirrors `range.value` → `select.value` + pulses `__wizardAutosavePulse()` so drags autosave immediately; (c) hold-to-confirm intercepts `click`/`pointerdown`/`keydown Enter                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Space`, swaps the button label through idle→progress→done, and only calls `form.submit()`once a 1.2 s`pointerdown`timer elapses — cancels on`pointerup`/`pointerleave`/`blur`to make accidental publishes near-impossible. Script wired into`_chrome.eta`under the existing`wizardV2ShellEnabled`flag. Shell body`\_admin_wizard_step_body.eta`include dispatcher extended: steps 1–3, 4, 8 now resolve to`./\_wizard_step_<n>`in the v2 folder; 5, 6, 7, 9 continue to resolve to`../_wizard_step_<n>`(the step-9 review section is untouched — the hold-confirm lives on the publish form which is in the shell body, not the partial). CSS additions (~40 lines in`src/static/site.css`): `.wizard**char-counter`(right-aligned micro ink),`.wizard**slider-field input[type='range']`with`accent-color: var(--color-brand-500)`, `.wizard\_\_publish-btn[data-wizard-hold-confirm]`+`[data-holding]::after`progress fill via a 1200 ms`wizardHoldProgress`keyframe that reduces to an instant fill under`prefers-reduced-motion: reduce`. New test suite `tests/http/wizard-v2/step-4-9.test.ts`(7 cases): step 4 renders the char-counter with the server-initial count + flag-off does not, step 8 renders the range slider coupled with the hidden select + all three source-chip values, step 9 publish button ships the hold-confirm data-attrs + flag-off is clean,`wizard_step_enhancements.js`is served and linked from the step page. Final: 93 files / 838 tests passing (full`npm run check`green post-prettier auto-format). Unblocks chunk 2.5t (motion + copy pass + axe-core sweep over the v2 wizard + manual walkthrough + flip`WIZARD_V2_ENABLED`default to`true`). |
| 2026-04-19 | TD     | **Chunk 2.5t shipped — the seventh and final chunk closes the wizard redesign; `WIZARD_V2_ENABLED` default flipped from `false` to `true` in `src/config.ts`.** Copy + motion polish: per-step subtitle appended to the step counter (e.g. "Step 5 of 9 · Configure the widget") via a new `STEP_SUBTITLES` map and `.wizard__step-subtitle` span (light-on-surface ink-2, non-uppercase) in `src/templates/v2/_admin_wizard_step_body.eta`; softened hint copy on steps 4 and 8; `.wizard__chip` hover-lift (`translateY(-1px)`) and design-token transitions (`--duration-fast --ease-standard`) so the chip primitive feels responsive without chattering under `prefers-reduced-motion: reduce` (which the global block in `design-tokens.css` already flattens). axe-core sweep: new `tests/browser/axe-wizard-v2.test.ts` (10 cases) seeds a fully-populated draft, spins up a Playwright context under `WIZARD_V2_ENABLED=1`, and runs axe over the drafts list + all 9 step pages. Fixes surfaced by the sweep: step-subtitle contrast raised from `--color-ink-4` (fails AA at ~3:1) to `--color-ink-2` (passes); the hidden `<select>` behind the difficulty slider gets `tabindex="-1"` in `wizard_step_enhancements.js` so axe's `aria-hidden-focus` rule stops flagging (aria-hidden on a focusable element is a trap); publish button background moved from `--color-accent-warm-500` (`#c2892a`, 3.04:1 on white — fails) to `--color-accent-warm-700` (`#7a5414`, ≈8:1 — passes) plus `font-weight: 600`. Preview pane excluded from the axe run (`.exclude('[data-wizard-preview]').exclude('[data-wizard-preview-pane]')`) because the pupil-widget dispatcher's pre-existing lack of individual input labels is out of scope for wizard-redesign work — those widgets are tested against the pupil-facing surfaces in `tests/browser/axe.test.ts`. New shell test asserts the step-subtitle is present on step 1 ("Home for the question") and step 9 ("Try it, then publish"). v1 regression guard: `tests/http/wizard-steps.test.ts` now sets `WIZARD_V2_ENABLED=0` in its `beforeEach` (and cleans up in `afterEach`) so it continues to exercise the v1 template set during the grace window; the v2-default flip would otherwise break assertions like "My question drafts" (v1) vs "Your question drafts" (v2) and `admin-wizard__preview-stage` (v1-only class). Every existing v2 test suite (`shell`, `drafts-list`, `autosave`, `shortcuts`, `step-1-3`, `step-4-9`) explicitly pins `WIZARD_V2_ENABLED=1` already, so the flip left them green. `src/config.ts` docstring updated to mark the grace-window semantics ("kept around for a short grace window so the flag can be flipped back if a regression surfaces in the first week of classroom use"). Final: 94 files / 849 tests passing (full `npm run check` green post-prettier auto-format). **Phase 2.5 wizard redesign (chunks 2.5n–2.5t) is now complete.** This satisfies the Phase 3 gate in §10 of `WIZARD_REDESIGN_PLAN.md`: flag is on, axe is green, and the teacher end-to-end walkthrough is the remaining manual sign-off the user requested to run themselves after the combined Phase 2 + 2.5 lesson test.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
