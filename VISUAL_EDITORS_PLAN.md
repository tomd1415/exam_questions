...
Survey of every `expected_response_type` in the wizard step-5 editor, identifying where the teacher still has to hand-author pipe-encoded text and where a visual editor would materially improve authoring. Scope is _authoring_ only — the pupil-facing widgets already render visually.
...
### multiple*choice / tick_box / matching / matrix_tick*\* — done-ish
...
- A WYSIWYG preview of the pupil-facing rendering for _every_ widget — cheap for some (matrix, matching) and expensive for others (logic_diagram). Decide per-chunk, not globally.
- Merging the pupil renderer and the editor canvas into one component. Coupling them makes both harder to change. The editor _reuses_ the renderer by composition.
