...
| 5   | In psql: `SELECT count(*) FROM questions WHERE similarity_hash LIKE 'curated:%' AND retired_at IS NULL;`                                        | Returns N. Drop the `retired_at` clause to also see archived rows from `content/curated/retired/` — those count toward the total but are hidden from pupils. |
| 6   | In psql: `SELECT approval_status, active, count(*) FROM questions WHERE similarity_hash LIKE 'curated:%' AND retired_at IS NULL GROUP BY 1, 2;` | Single row: `approved, true, N`.                                                                                                                             |
