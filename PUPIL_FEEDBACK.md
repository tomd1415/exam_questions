...
| ---------- | ------ | ------- | ---------------------------------------------------------------------------------------------------- | ------------ | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------- |
| 2026-04-18 | pupils | phase 2 | Trace tables render as a textarea with a `\|` hint; should be a fixed-size grid with per-cell inputs | change feat. | yes               | New widget for `trace_table`: header row from the question_part config, one input per cell, serialise to the existing `raw_answer` on autosave so marker logic unchanged                    | resolved | (chunk 2.5d)      |
| 2026-04-18 | pupils | phase 2 | Matching answers (draw lines / pair items) render as a textarea                                      | new feature  | yes               | Add `matching` expected_response_type: left column prompts, right column drop targets; serialise to `left_label=right_label` lines                                                          | resolved | (chunk 2.5e)      |
| 2026-04-18 | pupils | phase 2 | Fill-in-the-missing-word questions with a word bank are not supported                                | new feature  | yes               | Add `cloze_with_bank` widget: prompt contains `___` placeholders, a shared list of distractors is shown, pupil drags/clicks to fill                                                         | resolved | (chunk 2.5c)      |
| 2026-04-18 | pupils | phase 2 | Fill-in-the-missing-word questions without a word bank are not supported                             | new feature  | yes               | Add `cloze_free` widget: prompt contains `___` placeholders rendered as inline inputs; autosave serialises to `1=foo\n2=bar`                                                                | resolved | (chunk 2.5c)      |
| 2026-04-18 | pupils | phase 2 | Drawing logic diagrams is not supported                                                              | new feature  | maybe             | MVP shipped: free-draw canvas (pen + eraser + clear) saving a PNG, teacher-marked. Structured wire model deferred to Phase 3 under the same `logic_diagram` type via a new `variant`        | resolved | (chunk 2.5f MVP)  |
| 2026-04-18 | pupils | phase 2 | Filling in blank labels on diagrams is not supported                                                 | new feature  | yes               | MVP shipped: pupil widget + deterministic marker for `diagram_labels` (image + hotspot inputs). Teacher upload route + audit event land in chunk 2.5j (wizard) where the only caller exists | resolved | (chunk 2.5g MVP)  |
