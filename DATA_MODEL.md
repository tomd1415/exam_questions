...
| `widget_tips_dismissed` | JSONB NOT NULL DEFAULT `'{}'::jsonb` | Per-user record of which first-encounter widget tips the user has dismissed. Keys are widget registry keys (e.g. `flowchart`), values are ISO timestamps. Pupil-facing `/me/widget-tips/dismiss` updates this (migration 0024) |
| `theme_preference`      | TEXT NOT NULL DEFAULT `'auto'`       | `light` \| `dark` \| `auto`. Chosen from the account menu; `auto` follows the OS. CHECK enforced in DB (migration 0026)                                                                                                        |
...
| `marks_total`               | INT                           |                                                                                                                                                                                                                                                                                                                                                                                                       |
| `expected_response_type`    | TEXT                          | Widget registry key. See `src/lib/widgets.ts` for the authoritative list; Phase 1: `multiple_choice`, `tick_box`, `short_text`, `medium_text`, `extended_response`, `code`, `algorithm`, `trace_table`. Phase 2.5 additions: `matrix_tick_single`, `matrix_tick_multi`, `cloze_free`, `cloze_with_bank`, `cloze_code`, `trace_table_grid`, `matching`, `logic_diagram`, `diagram_labels`, `flowchart` |
| `model_answer`              | TEXT                          |                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `retired_at`                | TIMESTAMPTZ NULL              | Soft-retirement: approved questions taken out of rotation without losing historical attempt data. Partial index `questions_active_live_idx` is `(active) WHERE active = true AND retired_at IS NULL` so the hot picker only sees live rows (migration 0027)                                                                                                                                           |
...
| `pilot_shadow_status`    | TEXT NULL                 | `pending_shadow` \| `reviewed`. Populated only while `LLM_MARKING_PILOT=true`. Surfaces the row in the pilot-shadow queue at `/admin/moderation?mode=pilot` in addition to its usual moderation flow; flipped to `reviewed` when the teacher records their parallel mark. Orthogonal to `moderation_status` — gate-clean pilot rows stay live-visible to the pupil (migration 0033) |
| `created_at`             | TIMESTAMPTZ               |                                                                                                                                                                                                                                                                                                                                                                                     |
