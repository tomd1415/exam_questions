...
### 5.3 Running the prompt eval harness (chunk 3h)

The nightly prompt eval harness replays every `active` marking prompt against the golden fixture set under [prompts/eval/](prompts/eval/) and writes a pass/fail report. The admin page at `/admin/evals/latest` always renders the newest JSON report on disk.

```bash
# Dry run: load fixtures, route through a stub marker, write a report,
# exit non-zero on any fail. Does not call OpenAI — safe in CI and on
# any machine without an OPENAI_API_KEY.
EVAL_DRY_RUN=1 npm run eval

# Real run: requires LLM_ENABLED=true and a valid OPENAI_API_KEY. Uses
# the same LlmOpenResponseMarker the dispatch pipeline uses, so what
# passes here passes in production.
npm run eval
```

Reports land in [scripts/eval/out/](scripts/eval/out/) as a pair of files per invocation: `{timestamp}.json` (authoritative, consumed by the admin page) and `{timestamp}.md` (human-readable, consumed by CLI and CI job summaries).

Exit codes:

- `0` — every fixture passed.
- `1` — at least one fixture failed (CI can gate prompt promotions on this).
- `2` — uncaught exception before scoring (typically a fixture schema error or DB/OpenAI connectivity issue).

Point the admin page at a non-default directory via `EVAL_OUT_DIR=/path/to/out` in the process env — this is how the HTTP tests point the route at a temp dir without rebuilding the app.
